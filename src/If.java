...
//        가장 많이 사용되는 조건문으로 조건식의 결과가 참일경우 해당코드 블럭을 실행하는 형태의 조건문
//        단순 if문 : 조건식의 결과가 참 일 경우 코드블럭의 내용을 추가로 실행하는 형태
...
//        if ~ else 문 : 조건식의 결과가 참 일경우 바로 아래의 코드블럭을 실행하고 결과가 거짓일 경우 else 아래의 코드블럭을 실행하는 형태
//        단순 if문 과는 다르게 둘중 하나는 선택해야 하는 조건문

        System.out.println("탕수육 먹는 형태는 입력하세요");
        System.out.print("찍먹? 부먹? : ");
        String input = scan.next(); //문자열로 키보드 입력 받음

//        equals(문자열) : 지정한 문자열과 동일한 문자열인지 확인, true/false 로 출력
        if (input.equals("부먹")){
...
//        else if 문 : 조건식이 여러개가 존재하며 조건식의 결과가 참인 코드블럭을 실행하는 형태
//        조건식을 여러개 사용하기 떄문에 단계별로 조건식을 설정해야 함(문법적으로는 문제가 없으니 논리적으로 문제가 발생 할수 있음)
//        마지막 존재하는 else 구문은 생략 가능 (else 구문으로 갈 수 없도록 모든 조건을 다 입력했을 경우,
//        else 구문으로 빠지는 경우에 그냥 아무런 처리를 하지 않는 경우)
...
//        }
        ...
        System.out.println("\n----- 중첩 if문 -----\n");
//        중첩 if문 : if, if ~ else, else if 문의 코드 블럭에 if문을 사용하는 방식
